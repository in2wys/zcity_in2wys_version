...
function MODE:EndRound()
    ...
    elseif zb.rtype == "hostage" then
        ...
        if IsValid(zb.hostage) and not zb.hostage.organism.alive then
            ...
            if zb.HarmDoneDetailed[zb.hostage:EntIndex()] then
                ...
                PrintMessage(HUD_PRINTTALK, (maxTeam == 0 and "Seperatists" or "АЗОВ") .. " have killed the hostage")
            else
                winner = 3
            end
        end
    ...
    local winnerprt = (winner == 1 and "АЗОВ") or (winner == 0 and "Seperatists") or "Nobody"
	...
	if winsTeam0 > winsTeam1 then
		for _, ply in ipairs(team.GetPlayers(1)) do
			...
			ply:ChatPrint("We're falling behind! Sending finance bonus of 1000.")
		end
